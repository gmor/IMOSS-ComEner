...
```{r, echo=F, results=F, message=F, warning=F}
if(length(edistribucion_files)>0) {
...
coefs_CUPS <- df_ly %>% 
  group_by(CUPS) %>% 
  summarise(coef=mean(consumption))
...
write.table(df_ly_with_gen,
            paste0(wd,"/../Resultats/Resultats de resum/Dades horaries - coeficients estàtics.csv"),
            quote=F, row.names = F, col.names = T, sep=";",dec=",")
```
...
df_ly_with_gen_summary <- df_ly_with_gen %>% 
  group_by(CUPS) %>%
  summarise(
    "Demanda anual (kWh)" = round(sum(consumption,na.rm=T),2),
    "Generació anual (kWh)" = round(sum(generation,na.rm=T),2),
    "Autoconsum (%)"=
      round((sum(selfcons,na.rm=T)/sum(generation,na.rm=T))*100,2),
    "Excedent (%)"=
      round(((sum(generation,na.rm=T)-sum(selfcons,na.rm=T))/
               sum(generation,na.rm=T))*100,2),
    "Importat de xarxa (%)"=round((-sum(fromgrid,na.rm=T)/
                  sum(consumption,na.rm=T))*100,2)
    )
df_ly_with_gen_summary
write.table(df_ly_with_gen_summary,
            ...
            quote=F, row.names = F, col.names = T, sep=";")
write.table(df_ly_with_gen,
            paste0(wd,"/../Resultats/Resultats de resum/Dades horaries - coeficients dinàmics.csv"),
            quote=F, row.names = F, col.names = T, sep=";",dec=",")
