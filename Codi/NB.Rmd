...
```{r}
ggplot(generation_df %>% filter(NomCampFV=="TotalFV" & altitude>0) %>% 
         group_by(azimuth=round(azimuth,0),altitude=round(altitude,0),NomCampFV) %>%
         summarise(
           generation = mean(generation,na.rm=T)
         ) %>% ungroup(), aes(azimuth, altitude, col=generation)) +
  geom_line() +
  scale_fill_distiller(palette = "Spectral", direction = -1) +
  ylim(c(90,0)) + xlim(c(0,360)) +
  coord_polar()
